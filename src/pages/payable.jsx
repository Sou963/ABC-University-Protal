...
const PayableManager = () => {
  ...
  const grandRawTotal = Object.values(groupedData).reduce(
    (sum, item) => sum + item.rawPayable,
    0
  );
        ...
        <div className="row g-0">
          <div className="col-md-8 p-4 bg-white">
            <h4 className="fw-bold mb-1 text-dark">
              <i className="bi bi-wallet2 text-primary me-2"></i>Financial
              Account Statement
            </h4>
            <p className="text-muted mb-0 small">
              Student ID:{" "}
              <span className="fw-bold text-primary">{loggedInStudentId}</span>{" "}
            ...
            {totalWaiverPercent > 0 && (
              <div className="mt-3">
                <span className="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2 border border-success border-opacity-25">
                  <i className="bi bi-stars me-1"></i> Active Scholarship:{" "}
                  {totalWaiverPercent}% Off
                </span>
              </div>
            )}
          </div>
          <div className="col-md-4 p-4 bg-primary d-flex flex-column justify-content-center text-white text-md-end">
            <small className="opacity-75 text-uppercase fw-bold tracking-wider">
              Net Payable Amount
            </small>
        ...
        <div className="col-lg-8">
          {loading ? (
            ...
            sortedLevels.map((lvl) => (
              <div
                key={lvl}
                className="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden animate-fade-in"
              >
                <div className="card-header bg-white py-3 border-bottom d-flex justify-content-between">
                  <span className="fw-bold">
                    <i className="bi bi-layers text-primary me-2"></i>Level:{" "}
                    {lvl}
                  </span>
                  <span className="text-muted small">
                    Semester Subtotal: ৳{" "}
                    {groupedData[lvl].rawPayable.toLocaleString()}
                  </span>
                </div>
          ...
          >
            ...
            <div className="d-flex justify-content-between mb-3">
              ...
              <span className="fw-bold text-dark">
                ৳ {grandRawTotal.toLocaleString()}
              </span>
            </div>

            <div className="d-flex justify-content-between mb-3 align-items-center">
              <span className="text-muted small">
                Applied Waiver ({totalWaiverPercent}%)
              </span>
              <span className="badge bg-success bg-opacity-10 text-success">
                - ৳ {totalWaiverAmount.toLocaleString()}
              </span>
            </div>
            ...
            {waivers.length > 0 && (
              <div className="bg-light p-3 rounded-3 mb-4">
                <p className="x-small fw-bold text-uppercase text-muted mb-2">
                  Benefit Details
                </p>
                {waivers.map((w, idx) => (
                  <div
                    key={idx}
                    className="d-flex justify-content-between x-small mb-1"
                  >
            ...
            <button
              className="btn btn-primary w-100 rounded-pill py-3 fw-bold shadow-sm mb-2"
