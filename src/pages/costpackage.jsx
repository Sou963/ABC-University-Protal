...
const CostPackage = () => {
  ...
  const fetchPackageData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        apiUrl(`/api/package/student/${loggedInStudentId}`)
      );
  ...
  useEffect(() => {
    fetchPackageData();
  }, [fetchPackageData, loggedInStudentId]);
      ...
      <div className="container py-5">
        {loading ? (
          ...
          packages.map((pkg, idx) => (
            <div
              key={idx}
              className="row g-0 shadow-lg rounded-5 overflow-hidden animate-entry"
            >
              ...
              <div className="col-lg-4 bg-primary p-5 text-white d-flex flex-column justify-content-between">
                <div>
                  ...
                  <h6 className="text-uppercase opacity-75 tracking-widest small fw-bold">
                    Active Package
                  </h6>
                ...
                <div className="mt-5">
                  <div className="p-3 bg-white bg-opacity-10 rounded-4 border border-white border-opacity-10">
                    ...
                    <h3 className="fw-bold mb-0">
                      {pkg.currency === "BDT" ? "৳" : "$"}{" "}
                      {pkg.costRates
                        .reduce((acc, curr) => acc + curr.costAmount, 0)
                        .toLocaleString()}
                    </h3>
                  </div>
                  <p className="mt-4 small opacity-50 fst-italic">
                    * This amount represents the base cost before any applicable
                    waivers.
                  </p>
                </div>
              </div>
              ...
              <div className="col-lg-8 bg-white p-4 p-md-5">
                <div className="d-flex justify-content-between align-items-center mb-4">
                  <div>
                    ...
                    <p className="text-muted small mb-0">
                      Student: {loggedInStudentName} ({loggedInStudentId})
                    </p>
                  </div>
                  <button
                ...
                <div className="list-group list-group-flush mt-2">
                  {pkg.costRates.map((rate, rIdx) => (
                    <div
                      key={rIdx}
                      className="list-group-item px-0 py-4 d-flex justify-content-between align-items-center bg-transparent border-bottom"
                    >
                      ...
                      <div className="text-end">
                        <h6 className="fw-bold text-primary mb-0">
                          {pkg.currency === "BDT" ? "৳" : "$"}{" "}
                          {rate.costAmount.toLocaleString()}
                        </h6>
                      </div>
                    </div>
                  ))}
